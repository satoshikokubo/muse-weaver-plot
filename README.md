...
Genre packs are available on [Booth](https://kokubox.booth.pm/).

**Installation:**

1. Open the plugin settings and click **Open Folder** — the `MuseWeaver/PluginSettings/MWPlot/` folder will be created automatically.
2. Place the downloaded JSON file in that folder.
3. Click **Reload** in the plugin settings.

> **If you use Obsidian Sync:** Enable **Sync all other file types** on every device (Settings → Sync → Selective sync). Without this, pack JSON files will not sync.
...
追加ジャンルパックは [Booth](https://kokubox.booth.pm/) で購入できます。

**導入手順：**

1. プラグイン設定を開き、**フォルダを開く** をクリック — `MuseWeaver/PluginSettings/MWPlot/` フォルダが自動作成されます。
2. ダウンロードしたJSONファイルをそのフォルダに配置してください。
3. プラグイン設定の **再読み込み** をクリック。

> **Obsidian Syncをご利用の場合：** 設定 → 同期 → 選択的同期 の **「他のすべてのタイプのファイルを同期」** をPC・スマホ両方でONにしてください。この設定がOFFだとパックJSONが同期されません。
